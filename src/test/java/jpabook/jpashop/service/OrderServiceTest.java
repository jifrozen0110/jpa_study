...
public class OrderServiceTest {
	...
	public void 주문취소() throws Exception{
		...
		assertEquals("주문 취소시 상태는 CANCLE 이다.",OrderStatus.CANCEL,order.getOrderStatus());
		assertEquals("주문이 취소된 상품은 그만큼 재고가 증가해야 한다.",10,jpa.getStockQuantity());
	}
	...
	private Member createMember() {
		...
		member.setName("회원1");
		member.setAddress(new Address("서울", "rk", "123"));
		em.persist(member);
		return member;
	}
